...
#define REVISION_RULESET 1
	...
	#define RENDER_MAX_SAMPLES 64

	#define RENDER_PROGRESSIVE 1
    #define LIVE_RELOAD 1
    #if LIVE_RELOAD
        #undef RENDER_MAX_TIME_MS
        #define RENDER_MAX_TIME_MS 1e9 // We don't want the rendering to pause when live editing.
    #endif
#endif
	...
	// party release config, based on Revision ruleset
		...
		#define RENDER_MAX_TIME_MS 99000

		#define RENDER_EXACT_SAMPLES 64 // without the constraints of party rules, we can set an exact quality bar, if we want.
